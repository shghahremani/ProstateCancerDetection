...
def Radboud_branch(levels,n_classes,dropout_val,axis,filters):
    [f1, f2, f3, f4, f5, center] = levels
    OUTPUT_MASK_CHANNELS = n_classes
    print(center.shape)
    center = double_conv_layer(center, 32 * filters, 'center')
...
def Karolinska_branch(levels,n_classes,dropout_val,axis,filters):
    [f1, f2, f3, f4, f5, f1_center, f2_center, f3_center, f4_center, f5_center] = levels
    OUTPUT_MASK_CHANNELS = n_classes

    print(f2_center.shape)
    center = double_conv_layer(f2_center, 32 * filters, 'center')
    # print('center is {0}'.format(center.shape))
    gate_signal = gating(center, 32 * filters)
    print(gate_signal.shape)
    # print('gate_signal is {0}'.format(gate_signal.shape))
    g_conv2, att2 = grid_attention(f2, gate_signal, inter_channels=16 * filters)
    print(g_conv2.shape)
    ...
    up_14 = double_conv_layer(up_14, filters * 16, layer_name)
    # print('up_14 is {0}'.format(up_14.shape))
    #g_conv3, att3 = grid_attention(f1, up_14, inter_channels=8 * filters)
    up_28 = concatenate([UpSampling2D(size=(2, 2))(up_14), f1], axis=axis)
    layer_name = 'second_upconv_layer'
    up_28 = double_conv_layer(up_28, filters * 8, layer_name)
    print(layer_name, up_28.shape)
    ...
    #up_conv_224 = dsv(up_224, OUTPUT_MASK_CHANNELS, 1, dropout_val)
    #final = concatenate([up_conv_14, up_conv_28, up_conv_56, up_conv_112, up_conv_224], axis=axis)
    final = concatenate([up_conv_14, up_conv_28], axis=axis)
...
def _unet(n_classes_Radboud,n_classes_Karolinska, encoder, l1_skip_conn=True, input_height=480,
          input_width=640):
    filters = 16
    axis = 3
    dropout_val = 0.2
    ...
    Radboud_maps= Radboud_branch(levels, n_classes_Radboud, dropout_val, axis, filters)
    Karolinska_maps= Karolinska_branch(levels, n_classes_Karolinska, dropout_val, axis, filters)
    ...
    return img_input, Radboud_maps,Karolinska_maps
