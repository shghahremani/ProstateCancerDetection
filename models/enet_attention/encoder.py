...
def build(inp, dropout_rate=0.01):
    ...
    for _ in range(4):
        enet_att_input = bottleneck(enet, 64, dropout_rate=dropout_rate)  # bottleneck 1.i

    enet = bottleneck(enet_att_input, 128, downsample=True)  # bottleneck 2.0
    ...
    for _ in range(2):
        ...
        enet_gate = bottleneck(enet, 128, dilated=16)  # bottleneck 2.8
    return enet_gate,enet_att_input
