...
def samplewise(x,samplewise_center=False,samplewise_std_normalization=False,rescale=1.0,**kwargs):

    channel_index=2;
    # x=x*rescale
    samplewise_standardize_axis = channel_index
    if type(samplewise_standardize_axis) is int:
        samplewise_standardize_axis = (samplewise_standardize_axis,)
...
def random_adapt_hist(x,sync_seed=None,**kwargs):
    np.random.seed(sync_seed)
    if np.random.rand(1) >0.5:
        clp=np.random.rand(1)+0.5
        img_adapteq = np.array(x)
        clahe = cv2.createCLAHE(clipLimit=clp, tileGridSize=(8, 8))
        for i in [0, 1, 2]:
            cl1 = clahe.apply(img_adapteq[:, :, i])
            img_adapteq[:, :, i] = cl1
    else:
        img_adapteq=x
    return img_adapteq
...
def resize_image(x,target_size=None,**kwargs):
    x=cv2.resize(x,target_size,interpolation = cv2.INTER_CUBIC)
    # from PIL import Image
    # x=np.array(Image.fromarray(x).resize((target_size[1], target_size[0])))
    return x
...
class ImageDataGenerator(object):
    ...
    def process(self, x):
        ...
        self.__sync_seed = np.random.randint(0, 4294967)
...
class Iterator(object):
    ...
    def __add__(self, it):
        ...
        seed = self.seed or np.random.randint(0, 4294967)
...
class ListArrayIterator(Iterator):
    ...
    def __init__(self, X, y, image_data_generator,
        ...
        self.save_format = save_format
        seed = seed or image_data_generator.config['seed']
        super(ListArrayIterator, self).__init__(self.X_sample, batch_size, shuffle, seed)

    def __add__(self, it):
        if isinstance(it, ListArrayIterator):
            assert  self.X_sample == len(it.X)
        if isinstance(it, DirectoryIterator):
            assert self.X_sample == it.nb_sample
        it.image_data_generator.sync(self.image_data_generator)
        return super(ListArrayIterator, self).__add__(it)
